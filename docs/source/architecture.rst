...
Новые программные модули **voice2text** и **email_gw** размещены на виртуальных машинах совместно с существующими компонентами обработки сигнализации и медиатрафика и 
...
1. При поступлении вызова с переадресацией на сервис **VoiceBox** модуль **sip_processing** выполняет проверку статуса подключения услуги для вызываемого абонента в базе данных **VoiceDB**.
2. При активной услуге медиатрафик передается через модуль **rtp_processing** в модуль **voice2text** для выполнения распознавания речи и формирования текстовой версии голосового сообщения.
3. Полученные результаты транскрибации, а также данные о вызывающем и вызываемом абонентах передаются в модуль **email_gw**. Модуль **email_gw** запрашивает адрес электронной почты абонента в базе данных **VoiceDB** и направляет подготовленные данные в почтовую систему оператора, посредством интеграционного интерфейса **EmailAPI**. 
